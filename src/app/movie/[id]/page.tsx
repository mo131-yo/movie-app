...
export const fetchMovieById = async (id: string) => {
  ...
  return (
    ...
    <div className="flex justify-between">
      <div className="flex w-52.75 mt-2 font-semibold">
...
<div className="flex gap-8 justify-center">
    <Image
      ...
      width={300}
      height={450}
      className="w-72.5 h-107 object-cover rounded-xl bg-gray-200 transition-transform duration-500 hover:scale-105"
    />
</div>
  <div className="relative w-190 h-107 overflow-hidden rounded-lg ">
  <Image
    src={
      movie.poster_path
...
{movie.genres?.length > 0 && (
  <div className="flex flex-wrap gap-2 mt-3">
    {movie.genres.map((genre: { id: Key | null | undefined; name: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; }) => (
      <Link
        key={genre.id} href={`/genre/${genre.id}`} className="px-3 py-1 text-sm border rounded-full hover:bg-gray-100 transition">
        {genre.name}
      </Link>
    ))}
  </div>
     ...
     <div className="flex justify-between pt-10 pb-8">
       <h3 className="font-semibold text-xl md:text-2xl text-black lg:px-10 md:px-12 pb-5">More like this</h3>
