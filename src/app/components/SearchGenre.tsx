...
export default function GenrePage() {
  ...
  return (
    ...
    //   <Badge onClick={() => setOpen(!open)} variant="outline" className="cursor-pointer shadow text-black text-sm font-medium px-4 py-2 mt-4 rounded-lg mb-4 flex gap-2 items-center hover:bg-gray-50 transition-colors dark:bg-gray-800 dark:text-white">
    ...
    <div className="relative px-4 sm:px-6 lg:px-20 py-4 sm:py-8">
  <Badge 
    onClick={() => setOpen(!open)} 
    variant="outline" 
    className="inline-flex cursor-pointer shadow text-black text-sm font-medium px-4 py-2 mt-4 rounded-lg mb-4 gap-2 items-center hover:bg-gray-50 transition-colors dark:bg-gray-800 dark:text-white"
  >
    <SlArrowDown className={`transition-transform ${open ? "rotate-180" : ""}`} />
    Genre {selectedGenres.length > 0 && (
      <span className="ml-1 bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-[10px] dark:bg-blue-900 dark:text-blue-100">
        {selectedGenres.length}
      </span>
    )}
  </Badge>
  ...
  {open && (
    <div className="
      absolute z-50 mt-2 p-5
      left-4 right-4 w-auto 
      sm:left-6 sm:right-auto sm:w-112.5
      lg:left-20
    ...
    ">
      ...
      {loading ? (
        ...
        <>
          <div className="flex flex-wrap gap-2.5 max-h-[60vh] sm:max-h-72 overflow-y-auto pr-2 custom-scrollbar">
            {genres.map((genre) => {
              const isSelected = selectedGenres.includes(genre.id);
              return (
                <button 
                  ...
                  className={`px-4 py-2 text-xs sm:text-sm font-medium border rounded-full transition-all duration-200 ${
                    isSelected
                      ? "bg-blue-600 text-white border-blue-600 shadow-md transform scale-105"
                      : "bg-transparent text-gray-600 border-gray-300 hover:border-blue-400 hover:text-blue-500 dark:text-gray-400 dark:border-gray-700"
                  }`}
                >
                  {genre.name}
                </button>
          ...
          <div className="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4">
            <button 
              onClick={() => setSelectedGenres([])} 
              className="text-sm font-medium text-gray-400 hover:text-red-500 transition-colors w-full sm:w-auto"
            >
              Reset all
            </button>
            <div className="flex gap-2 w-full sm:w-auto">
               <button 
                ...
                className="flex-1 sm:flex-none px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-lg transition-colors dark:text-gray-300 dark:hover:bg-gray-800"
              >
                Cancel
              </button>
              <button 
                onClick={applyFilter} 
                className="flex-1 sm:flex-none px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 shadow-lg shadow-blue-200 dark:shadow-none transition-all"
              >
                Apply
              </button>
            </div>
